...
uniform float _MeshDistance = 2000;
...
//    float alpha = (-viewPos.z - _ProjectionParams.y) / _MeshDistance;
//    alpha = min(alpha, 1);
//    data.alpha = alpha;

    float cameraDistance = distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, v.vertex));

    if(cameraDistance < _MeshDistance)
        v.vertex.xyz = float3(v.vertex.x, v.vertex.y - 1000000, v.vertex.z);
