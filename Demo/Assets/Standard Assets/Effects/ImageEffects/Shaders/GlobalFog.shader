Shader "Hidden/GlobalFog"
{
    ...
    uniform float _SkyFogBlending;

    uniform sampler2D _MainTex;

    uniform sampler2D_float _CameraDepthTexture;
    //uniform sampler2D_float _LastCameraDepthTexture;
    //uniform sampler2D_float _CameraMotionVectorsTexture;
...
//      half4 Sky = tex2D(_Skybox, i.uv);
        ...
        //float rawDepth = SAMPLE_DEPTH_TEXTURE(_LastCameraDepthTexture,i.uv_depth);
        //float rawDepth = SAMPLE_DEPTH_TEXTURE(_CameraMotionVectorsTexture,i.uv_depth);
        ...
        if (AffectSkybox)
            //fogFac = fogFacSky;
            fogFac = lerp(fogFac, fogFacSky, _SkyFogBlending);
